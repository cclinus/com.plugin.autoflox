		...
		try {
			...
			while ((strLine = br.readLine()) != null) {
				if (strLine.startsWith("ppt")) {
					...
					String function_name = strLine.substring(strLine
							.lastIndexOf('.') + 1);
						...
						if (strLine.startsWith("ppt-type")) {
							...
							if (colonIndex >= 0) {
								if (colonIndex == name.length() - 1) {
									System.out
											.println("Error: malformed variable declaration");
									System.exit(-1);
								}
							...
							// First, take out the [..] in case the variable
							// is an array
							...
							if (var_scope.equals("local")) {
								...
								System.out
										.println("Error: malformed variable scope");
								System.exit(-1);
							}
							...
							VariableDesc var_desc = new VariableDesc(name,
									var_kind, dec_type, rep_type, isGlobal);

							// Do not add the variable if its name is
							// RCA_errorMsg and this is not the error trace
							String traceType = function_name
									.substring(function_name.lastIndexOf(":::"));
							if (!(!traceType.equals(":::ERROR") && name
									.equals("RCA_errorMsg"))) {
					...
					Collections.sort(function_decls,
							new FunctionDeclComparator());
					FunctionDecl new_fd = new FunctionDecl();
					new_fd.ppt_decl = pptDecl;
					int index = Collections.binarySearch(function_decls,
							new_fd, new FunctionDeclComparator());
