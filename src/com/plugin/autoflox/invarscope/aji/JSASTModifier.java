...
import com.plugin.autoflox.invarscope.aji.executiontracer.ProgramPoint;

//import com.crawljax.core.CrawljaxController;
//import com.crawljax.plugins.aji.executiontracer.ProgramPoint;

/**
 * Abstract class that is used to define the interface and some functionality
 * for the NodeVisitors that modify JavaScript.
 * 
...
public abstract class JSASTModifier implements NodeVisitor {
	...
	// Logger.getLogger(CrawljaxController.class.getName());
	...
	/**
	 * Contains the scopename of the AST we are visiting. Generally this will be
	 * the filename
	 */
	...
	protected JSASTModifier() {
		/*
		 * add -<number of arguments> to also make sure number of arguments is
		 * the same
		 */
	...
	/**
	 * Find out the function name of a certain node and return "anonymous" if
	 ...
	 * @return The new node.
	 */
	protected abstract AstNode createNode(FunctionNode function,
			String postfix, int lineNo);
	 ...
	 * @return The new node
	 */
	protected abstract AstNode createNode(AstRoot root, String postfix,
			int lineNo, int rootCount);

	/**
	 * Creates a node that can be inserted before and after a DOM modification
	 * statement (such as jQuery('#test').addClass('bla');).
	 * 
	 * @param shouldLog
	 *            The variable that should be logged (for example
	 *            jQuery('#test').attr('style'))
	 * @param lineNo
	...
	private AstNode makeSureBlockExistsAround(AstNode node) {
		...
		if (parent instanceof IfStatement) {
			/*
			 * the parent is an if and there are no braces, so we should make a
			 * new block
			 */
			...
			/*
			 * the parent is a while and there are no braces, so we should make
			 * a new block
			 */
			/*
			 * I don't think you can find this in the real world, but just to be
			 * sure
			 */
			...
			/*
			 * the parent is a for and there are no braces, so we should make a
			 * new block
			 */
			/*
			 * I don't think you can find this in the real world, but just to be
		...
		/**
		 * TODO: Frank, find a way to do this without concurrent modification
		 * exceptions.
		 */
	...
	public boolean visit(AstNode node) {
		...
		if (!((node instanceof FunctionNode || node instanceof ReturnStatement
				|| node instanceof SwitchCase || node instanceof AstRoot
				|| node instanceof ExpressionStatement
				|| node instanceof BreakStatement
				|| node instanceof ContinueStatement
				|| node instanceof ThrowStatement || node instanceof VariableDeclaration))) {// ||
																								// node
																								// instanceof
																								// ExpressionStatement
																								// ||
		...
		if (node instanceof FunctionNode) {
			...
			AstNode newNode = createNode(func, ProgramPoint.ENTERPOSTFIX,
					func.getLineno());
			...
			/* START FROLIN'S CODE */
			/*
			 * node = (AstNode) func.getBody().getFirstChild(); node = (AstNode)
			 * node.getNext(); //The first node is the node just added in front,
			 * so get next node int firstLine = 0; if (node != null) { firstLine
			 * = node.getLineno(); } while (node != null) { //Check if this
			 * child is the last line AstNode testNode = (AstNode)
			 * node.getNext(); if (testNode == null) { newNode =
			 * createNode(func, ProgramPoint.EXITPOSTFIX,
			 * node.getLineno()-firstLine+1);
			 * func.getBody().addChildrenToBack(newNode); break; }
			 * 
			 * if (node instanceof ReturnStatement || node instanceof
			 * IfStatement || node instanceof Block || node instanceof WhileLoop
			 * || node instanceof ForLoop || ) { node = (AstNode)
			 * node.getNext(); continue; }
			 * 
			 * newNode = createNode(func, ":::INTERMEDIATE",
			 * node.getLineno()-firstLine+1);
			 * func.getBody().addChildAfter(newNode, node);
			 * 
			 * //Get the node that's two positions after (since the next node
			 * will always be the newly added node) node = (AstNode)
			 * node.getNext(); node = (AstNode) node.getNext(); }
			 */
			/* END FROLIN'S CODE */

			node = (AstNode) func.getBody().getFirstChild();
			node = (AstNode) node.getNext(); // The first node is the node just
												// added in front, so get next
												// node
			int firstLine = 0;
			if (node != null) {
			...
			/*
			 * if this is not a return statement, we need to add logging here
			 * also
			 */
			if (!(node instanceof ReturnStatement)) {
				AstNode newNode_end = createNode(func,
						ProgramPoint.EXITPOSTFIX, node.getLineno() - firstLine
								+ 1);
			...
			while (curr_node != null) {
				AstNode peeked_node = (AstNode) curr_node.getNext(); // curr_node's
																		// parent
																		// will
																		// change,
																		// so
			...
			/*
			 * add an if statement prior to the catch block in case this is
			 * async call
			 */
			...
			if (!(async_if_tmp instanceof IfStatement)) {
				...
				AstNode async_marker = createNode(func, ":::ASYNC",
						func.getLineno());
			...
			if (rt.getSourceName() == null) { // make sure this is an actual
												// AstRoot, not one we created
				return true;
			}

			// this is the entry point of the AST root
			m_rootCount++;
			AstNode newNode = createNode(rt, ProgramPoint.ENTERPOSTFIX,
					rt.getLineno(), m_rootCount);
			...
			// also
			if (!(node instanceof ReturnStatement)) {
				AstNode newNode_end = createNode(rt, ProgramPoint.EXITPOSTFIX,
						node.getLineno() - firstLine + 1, m_rootCount);
				// add as last statement
			...
			cc.setVarName(catchVar);
			Block catch_b = new Block();
			AstNode newNode_try = createNode(rt, ":::ERROR", rt.getLineno(),
					m_rootCount);
			Name errorObj = new Name(0, "Error");
			StringLiteral errStr = new StringLiteral();
			errStr.setValue("Root Cause Analyzer: Error detected");
		...
		// else if (node instanceof BreakStatement || node instanceof
		// ConditionalExpression || node instanceof ContinueStatement || node
		// instanceof ExpressionStatement || node instanceof FunctionCall ||
		// node instanceof Assignment || node instanceof InfixExpression || node
		// instanceof ThrowStatement || node instanceof UnaryExpression || node
		// instanceof VariableDeclaration || node instanceof VariableInitializer
		// || node instanceof XmlDotQuery || node instanceof XmlMemberGet ||
		// node instanceof XmlPropRef || node instanceof Yield) {
			...
			if (node instanceof ThrowStatement) {
				...
				if (throw_s.getExpression() instanceof NewExpression) {
					NewExpression n_ex = (NewExpression) throw_s
							.getExpression();
			...
			if (node instanceof TryStatement) {
				return true; // no need to add instrumentation before try
								// statement anyway since we only instrument
			...
			if (func != null) {
				AstNode firstLine_node = (AstNode) func.getBody()
						.getFirstChild();
				if (func instanceof FunctionNode
						&& firstLine_node instanceof IfStatement) { // Perform
																	// extra
																	// check due
																	// to
																	// addition
				...
				if (func instanceof FunctionNode
						&& firstLine_node instanceof TryStatement) {
					TryStatement firstLine_node_try = (TryStatement) firstLine_node;
					firstLine_node = (AstNode) firstLine_node_try.getTryBlock()
							.getFirstChild();
				}
				firstLine_node = (AstNode) firstLine_node.getNext();
				int firstLine = 0;
				if (firstLine_node != null) {
					// If first child is an ExpressionStatement or
					// VariableDeclaration, then there might be multiple
					// instances of the instrumented node at the beginning of
					// the FunctionNode's list of children
				...
				if (node.getLineno() >= firstLine) {
					AstNode newNode = createNode(func, ":::INTERMEDIATE",
							node.getLineno() - firstLine + 1);
					// AstNode parent = node.getParent();
				...
				// if (firstLine_node instanceof IfStatement) { //Perform extra
				// check due to addition if statement
				// firstLine_node = (AstNode) firstLine_node.getNext();
				// }
				if (firstLine_node instanceof Block) {
					firstLine_node = (AstNode) firstLine_node.getFirstChild(); // Try
																				// statement
				}
				...
				if (node.getLineno() >= firstLine) {
					AstNode newNode = createNode(rt, ":::INTERMEDIATE",
							node.getLineno() - firstLine + 1, m_rootCount);
					// AstNode parent = node.getParent();
		...
		/*
		 * else if (node instanceof IfStatement) { func =
		 * node.getEnclosingFunction(); AstNode firstLine_node = (AstNode)
		 * func.getBody().getFirstChild(); firstLine_node = (AstNode)
		 * firstLine_node.getNext(); int firstLine = 0; if (node != null) {
		 * firstLine = firstLine_node.getLineno(); }
		 * 
		 * IfStatement if_node = (IfStatement) node;
		 * 
		 * //Instrument condition node AstNode cond_node = (AstNode)
		 * if_node.getCondition(); AstNode newNode = createNode(func,
		 * ":::INTERMEDIATE", cond_node.getLineno()-firstLine+1);
		 * if_node.addChildAfter(newNode, cond_node);
		 * 
		 * //Instrument thenPart AstNode then_node = (AstNode)
		 * if_node.getThenPart(); if (!(then_node instanceof ReturnStatement)) {
		 * if (then_node instanceof Scope) { AstNode scopeChild = (AstNode)
		 * then_node.getFirstChild(); while (scopeChild != null) { if
		 * (!(scopeChild instanceof ReturnStatement)) { newNode =
		 * createNode(func, ":::INTERMEDIATE",
		 * scopeChild.getLineno()-firstLine+1);
		 * 
		 ...
		 * 
		 * scopeChild = (AstNode) scopeChild.getNext(); scopeChild = (AstNode)
		 * scopeChild.getNext(); } else { scopeChild = (AstNode)
		 * scopeChild.getNext(); } } } else { newNode = createNode(func,
		 * ":::INTERMEDIATE", then_node.getLineno()-firstLine+1);
		 * if_node.addChildAfter(newNode, then_node); } }
		 * 
		 * //Instrument elsePart AstNode else_node = (AstNode)
		 * if_node.getElsePart(); if (else_node != null) { if (!(else_node
		 * instanceof ReturnStatement)) { if (else_node instanceof Scope) {
		 * AstNode scopeChild = (AstNode) else_node.getFirstChild(); while
		 * (scopeChild != null) { if (!(scopeChild instanceof ReturnStatement))
		 * { newNode = createNode(func, ":::INTERMEDIATE",
		 * scopeChild.getLineno()-firstLine+1);
		 * 
		 * //AstNode parent = makeSureBlockExistsAround(scopeChild);
		 ...
		 * scopeChild.getNext(); } else { scopeChild = (AstNode)
		 * scopeChild.getNext(); } } } else { newNode = createNode(func,
		 * ":::INTERMEDIATE", else_node.getLineno()-firstLine+1);
		 * if_node.addChildAfter(newNode, else_node); } } } }
		 */
		// else if (node instanceof IfStatement || node instanceof Block || node
		// instanceof ForLoop || node instanceof WhileLoop) {
		// func = node.getEnclosingFunction();
		...
		// }
			...
			if (func instanceof FunctionNode
					&& firstLine_node instanceof IfStatement) { // Perform extra
																// check due to
																// addition if
																// statement
				firstLine_node = (AstNode) firstLine_node.getNext();
			...
			if (firstLine_node != null) {
				// If first child is an ExpressionStatement or
				// VariableDeclaration, then there might be multiple instances
				// of the instrumented node at the beginning of the
				// FunctionNode's list of children
				while (firstLine_node != null) {
					firstLine = firstLine_node.getLineno();
					if (firstLine > 0) {
						break;
			...
			if (right_node != null) {
				Assignment assn_node = new Assignment(90, name_node,
						right_node, 0);
				ExpressionStatement expr_node = new ExpressionStatement(
						assn_node, true);

				// AstNode parent = makeSureBlockExistsAround(node);
				/*
				 * if (parent instanceof SwitchCase) { SwitchCase sc =
				 * (SwitchCase)parent; List<AstNode> statements =
				 * sc.getStatements(); List<AstNode> newStatements = new
				 * ArrayList<AstNode>();
				 * 
				 * Iterator<AstNode> it = statements.iterator(); while
				 * (it.hasNext()) { AstNode stmnt = it.next(); if
				 * (stmnt.equals(node)) { newStatements.add(expr_node); }
				 * newStatements.add(stmnt); } sc.setStatements(newStatements);
				 * }
				 */
				// else {
			...
			AstNode newNode = createNode(func, ProgramPoint.EXITPOSTFIX,
					node.getLineno() - firstLine + 1);

			// Need to wrap return call in block because otherwise, the added
			// node won't be executed (since the function has returned)
			// AstNode parent = makeSureBlockExistsAround(node);
			 ...
			 * Iterator<AstNode> it = statements.iterator(); while
			 * (it.hasNext()) { AstNode stmnt = it.next(); if
			 * (stmnt.equals(node)) { newStatements.add(newNode); }
			 * newStatements.add(stmnt); } sc.setStatements(newStatements);
			 * 
			 * return true; }
			 */
		...
		// objectAndFunction = mapper.get(node.toSource() + "-" +
		// arguments.size());
		// }
		...
		/*
		 * TODO: lineno will be off by one. otherwise we might have overlapping
		 * problems
		 */

		// parent.addChildBefore(createPointNode(objectAndFunction,
		// node.getLineno()),
		// getLineNode(node));
